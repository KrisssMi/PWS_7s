...
namespace _08_JsonRpc.Controllers
{
    ...
    public class JrController : ApiController
    {
        ...
        {
            return requests?
                .TakeWhile(_ => !ErrorExitActivated)    // пока не активирован ErrorExit
        ...
        private static JsonRpcResponce? ValidateRequest(JsonRpcRequest request)
        {
            ...
            if (method is null || methods.All(m => m != method))    // method is null or method is not in methods
            {
                return new JsonRpcResponce
                {
                    Id = null,
            ...
            var parametersAreNullAndMethodIsNotErrorExit = method != "ErrorExit" && parameters is null; // если параметры null и метод не ErrorExit
            var noKParameterAndMethodIsNotErrorExit = method != "ErrorExit" && string.IsNullOrEmpty(parameters!.k); // если параметр k null или пустая строка и метод не ErrorExit
            var noNParameterAndMethodIsNotErrorExitOrNotGetM = method is not ("ErrorExit" or "GetM") && parameters!.n is null;  // если параметр n null и метод не ErrorExit и не GetM
        ...
        private JsonRpcResponce ProcessRequest(JsonRpcRequest request)  // request is not null
        {
            ...
            if (method == "ErrorExit")
            {
                ...
                return new JsonRpcResponce
                {
                    Id = request.Id!,       // Id! обозначает что Id точно не null
                    Jsonrpc = jsonRpc!, 
                    Result = null,
